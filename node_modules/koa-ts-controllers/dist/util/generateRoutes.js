...
const argumentInjectorMap = {
    ...
    session: async (ctx, injectOptions) => {
        if (!ctx.session)
            throw boom.failedDependency('Sessions have not been activated on this server');
        if (typeof injectOptions === 'string') {
            return ctx.session[injectOptions];
        }
        return ctx.session;
    },
    ctx: async (ctx, injectOptions) => ctx,
...
async function _determineArgument(ctx, index, { injectSource, injectOptions }, type) {
    ...
    if (result && tools_1.isClass(type)) {
        ...
        if (errors.length > 0) {
            throw boom.badData('validation error for argument type: ' + injectSource, errors.map(it => {
...
async function _generateEndPoints(router, options, controller, parentPath, generatingForVersion) {
    ...
    lodash_1.default.each(actions, async (action, name) => {
...
async function generateRoutes(router, options, metadata) {
    ...
    lodash_1.default.each(metadata.controllers, async (controller) => {
        if (options.disableVersioning) {
            ...
            // e.g /api/v1/user
            lodash_1.default.each(options.versions, async (status, version) => {
